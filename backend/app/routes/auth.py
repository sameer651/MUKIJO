...
@router.post("/venue-owner/register", status_code=status.HTTP_201_CREATED)
def register_owner(payload: schemas.OwnerRegister, db: Session = Depends(get_db)):
    """Register a new venue owner profile into the database ecosystem."""
    email_clean = payload.email.replace(" ", "").lower() if payload.email else ""
    
    existing_owner = db.query(models.VenueOwner).filter(func.lower(models.VenueOwner.email) == email_clean).first()
    if existing_owner:
        raise HTTPException(status_code=400, detail="Email is already registered")
        
    # Note: Use your platform's secure password hashing function if available 
    new_owner = models.VenueOwner(
        username=payload.username,
        email=email_clean,
        phone_number=payload.phoneNumber,
        password_hash=payload.password  # Change to hash_password(payload.password) once helper is configured
    )
    db.add(new_owner)
    db.commit()
...
def login_owner(payload: schemas.OwnerLogin, db: Session = Depends(get_db)):
    """Authenticate a venue owner and return access tokens."""
    email_clean = payload.email.replace(" ", "").lower() if payload.email else ""
    
    owner = db.query(models.VenueOwner).filter(func.lower(models.VenueOwner.email) == email_clean).first()
    
    # Note: Replace text checking with your password hashing verification function down the line
    if not owner or owner.password_hash != payload.password:
        raise HTTPException(status_code=401, detail="Invalid email or password credentials")
        
    # If your JWT module is configured, generate an access token:
    # token = create_jwt_token(data={"sub": owner.email})
    # return {"access_token": token, "token_type": "bearer"}
    
    return {
        "message": "Login successful", 
        "access_token": f"mock-token-for-{owner.username}", 
        "token_type": "bearer"
    }
